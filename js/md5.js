...
function md5Bytes(inputBytes) {
  ...
  for (let i = 0; i < bytes.length; i += 64) {
    ...
    for (let j = 0; j < 64; j++) {
      ...
      D = C;
      C = B;
      B = add(B, rotateLeft(add(A, F, K[j], M[g]), S[j]));
      A = temp;
    }
...
function generateHash() {
  ...
  try {
    ...
    if (enableHmac.checked) {
      ...
      const innerHash = md5Bytes([...ipad, ...inputBytes]);
      hashBytes = md5Bytes([...opad, ...innerHash]);
    } else {
      const inputBytes = getInputBytes(text, inputEnc);
      hashBytes = md5Bytes(inputBytes);
    }
    ...
    outputSuccess.textContent = "MD5 hash generated successfully!";
  } catch (error) {
